...
**As-built authority**: Convergence measures code against spec, public contract, and saga tests (`npm test`). This repo has no chat UI or `domain:check-converge`.
...
- DOMAIN MODEL = `contracts/public/knowledge-index/` + `specs/NNN-*/spec.md` Domain Mapping
...
5. Append remediation tasks: extend saga module + tests; confirm `npm test` passes. If public behavior changed, bump `contracts/public/knowledge-index/` semver first.
